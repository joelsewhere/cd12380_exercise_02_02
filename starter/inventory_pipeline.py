...
# The variables names should align with the assets used in `eternal_dags.py`
# ---------------------------------------------------------------------------
...
def _write_summary(enriched: list[dict], date_str: str) -> None:
    ...
    df = pd.DataFrame([[len(enriched), date_str]])
    df.to_csv(stock_summary_asset.uri, mode='a', index=False, header=False)

    print(f"Writing {len(enriched)} enriched shipments for {date_str} to {stock_summary_asset.name}")
